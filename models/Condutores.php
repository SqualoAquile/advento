...
class Condutores extends model {
    ...
    public function nomeFerramentas($termo){
        // print_r($termo); exit;
        // echo "aquiiii"; exit;
        $array = array();
        // 
        $sql1 = "SELECT * FROM `ferramentas` WHERE situacao = 'ativo' AND ( descricao LIKE '%$termo%' OR tipo_ferramenta LIKE '%$termo%' ) ORDER BY descricao ASC";
        ...
        if($sql1->rowCount() > 0){  
            ...
            foreach ($nomesAux as $key => $value) {
                $nomes[] = array(
                    "id" => $value["id"],
                    "label" => $value["descricao"]." -- ".$value["tipo_ferramenta"],
